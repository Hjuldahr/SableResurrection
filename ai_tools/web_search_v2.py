...
End with a complete sentence and output only the paragraph.'''
        ...
        # Use an HTTP client to safely pull web links down into memory buffers
        with httpx.Client() as client:
            for image in images:
                ...
                try:
                    # If it's a standard web link, fetch the binary contents directly
                    if image.url.startswith(('http://', 'https://')):
                        res = client.get(image.url, timeout=5.0)
                        if res.status_code == 200:
                            b64_str = base64.b64encode(res.content).decode('utf-8')
                            prompt_images.append(b64_str)
                    else:
                        # Fallback context: If the layout maps to local file systems path files
                        prompt_images.append(image.url)
        ...
        response = ollama.chat(
            ...
            messages=[
                {
                    'role': 'system', 
                    'content': 'Write one informational paragraph that directly answers the query using the information provided.'
                },
                {
                    'role': 'user', 
                    'content': text_prompt, 
                    'images': prompt_images  # Natively processes local paths and raw b64 blocks
                }
            ],
            options={
...
if __name__ == "__main__": # only run test when called directly
    print(PageSummarizer().summarize_page("Why was the troupe of Monty Python created?", "https://en.wikipedia.org/wiki/Monty_Python"))